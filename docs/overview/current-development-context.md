...
사용자 승인으로 관리자·거래처·창고의 PC 상단 가로 메뉴를 왼쪽 사이드바로 통합했습니다. PC에서는 펼친 상태가 기본이며 아이콘 폭까지 접을 수 있고, 모바일은 기존 왼쪽 드로어를 유지합니다. 업무 API·데이터는 변경하지 않았습니다. 상세 내용과 검증 범위는 [PC 왼쪽 사이드바 내비게이션](../changes/2026-09-20-desktop-sidebar-navigation.md)을 참조합니다.
