...
## 2026-09-20 PC 사이드바 내비게이션

사용자 승인으로 관리자·거래처·창고의 PC 상단 가로 메뉴를 왼쪽 사이드바로 통합했습니다. PC에서는 펼친 상태가 기본이며 아이콘 폭까지 접을 수 있고, 모바일은 기존 왼쪽 드로어를 유지합니다. 경로 전환마다 인증 확인으로 전체 레이아웃이 로딩 화면으로 바뀌던 현상은, 인증 가드를 본문에만 적용하도록 조정해 해결했습니다. 업무 API·데이터는 변경하지 않았습니다. `176f3b4`의 GitHub CI 성공 뒤 Zorin 운영 서버 Web 이미지를 교체했으며, 내부와 `https://stm.approid.team`의 Web·API readiness가 모두 HTTP 200으로 확인됐습니다. 상세 내용과 검증 범위는 [PC 왼쪽 사이드바 내비게이션](../changes/2026-09-20-desktop-sidebar-navigation.md)을 참조합니다.
...
최종 갱신: 2026-09-20. 이번 갱신은 PC 사이드바 내비게이션 변경이며 기존 기능 검증 이력의 날짜는 아래에 별도로 표시합니다.
